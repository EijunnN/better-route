...
test.describe("Route Optimization Vehicle Assignment Debug", () => {
  test.beforeAll(async ({ request }) => {
    ...
    if (!loginResponse.ok()) {
      ...
      throw new Error(
        "Authentication failed - make sure the app is running and seeded",
      );
    }
  ...
  test("Debug: Inspect vehicles capacity and origin locations", async ({
    request,
  }) => {
    console.log("\n=== VEHICLE ANALYSIS ===");
    ...
    vehicles.forEach((v, index) => {
      ...
      console.log(`  ID: ${v.id}`);
      console.log(
        `  Weight Capacity: ${v.weightCapacity ?? "NOT SET (will default to 10000)"}`,
      );
      console.log(
        `  Volume Capacity: ${v.volumeCapacity ?? "NOT SET (will default to 100)"}`,
      );
      console.log(
        `  Max Orders: ${v.maxOrders} (NOT passed to VROOM's max_tasks!)`,
      );
      console.log(
        `  Origin: ${v.originLatitude}, ${v.originLongitude} (NOT USED - all vehicles use same depot)`,
      );
      console.log("");
    });
    ...
    const allSameCapacity = capacities.every(
      (c) =>
        c.weight === capacities[0].weight && c.volume === capacities[0].volume,
    );
  ...
  test("Debug: Inspect orders weight/volume requirements", async ({
    request,
  }) => {
    console.log("\n=== ORDER ANALYSIS ===");

    const response = await request.get(
      "/api/orders?status=PENDING&active=true&limit=100",
      {
        headers: {
          "x-company-id": companyId,
          Authorization: `Bearer ${authToken}`,
    ...
    const ordersWithWeight = orders.filter(
      (o) => o.weightRequired && o.weightRequired > 0,
    );
    const ordersWithVolume = orders.filter(
      (o) => o.volumeRequired && o.volumeRequired > 0,
    );
    ...
    orders.slice(0, 5).forEach((o, i) => {
      console.log(
        `  ${i + 1}. ${o.trackingId}: weight=${o.weightRequired ?? 0}, volume=${o.volumeRequired ?? 0}`,
      );
    });
    ...
    if (ordersWithWeight.length === 0 && ordersWithVolume.length === 0) {
      ...
      console.log(
        "Without capacity constraints, one vehicle can handle ALL orders.",
      );
    }
  });

  test("Debug: Run optimization and analyze vehicle distribution", async ({
    page,
    request,
  }) => {
    console.log("\n=== OPTIMIZATION TEST ===");
    ...
    const vehiclesResponse = await request.get(
      "/api/vehicles/available?limit=100",
      {
        headers: {
          "x-company-id": companyId,
          Authorization: `Bearer ${authToken}`,
    ...
    if (vehicles.length < 4) {
      console.log(
        `Only ${vehicles.length} vehicles available - need at least 4 for this test`,
      );
      test.skip();
      return;
    }
    ...
    result.routes.forEach((route) => {
      ...
      console.log(
        `  Total distance: ${(route.totalDistance / 1000).toFixed(2)} km`,
      );
    ...
    const vehiclesWithOrders = Object.values(vehicleAssignments).filter(
      (count) => count > 0,
    ).length;
    const totalOrdersAssigned = Object.values(vehicleAssignments).reduce(
      (a, b) => a + b,
      0,
    );
    ...
    if (vehiclesWithOrders === 1) {
      ...
      console.log(
        "1. All vehicles start from same depot (individual origins ignored)",
      );
      console.log("2. No max_tasks constraint passed to VROOM");
      console.log("3. Orders have no weight/volume requirements");
      console.log(
        "4. VROOM optimizes for minimum vehicles when unconstrained\n",
      );
      ...
      console.log(
        `PARTIAL ISSUE: Only ${vehiclesWithOrders} of ${selectedVehicleIds.length} vehicles got orders`,
      );
    } else {
      console.log(
        `OK: All ${vehiclesWithOrders} selected vehicles received orders`,
      );
    ...
    const minOrders = Math.min(
      ...Object.values(vehicleAssignments).filter((c) => c > 0),
      0,
    );

    console.log(`\nDistribution Analysis:`);
    console.log(
      `  Average orders per vehicle: ${avgOrdersPerVehicle.toFixed(1)}`,
    );
    console.log(`  Max orders on one vehicle: ${maxOrders}`);
    console.log(`  Min orders on one vehicle: ${minOrders}`);
    console.log(
      `  Distribution ratio: ${maxOrders > 0 ? (maxOrders / avgOrdersPerVehicle).toFixed(2) : "N/A"}x average`,
    );
  ...
  test("Debug: Simulate VROOM request to inspect payload", async ({
    request,
  }) => {
    ...
    // Get vehicles
    const vehiclesResponse = await request.get(
      "/api/vehicles/available?limit=4",
      {
        headers: {
          "x-company-id": companyId,
          Authorization: `Bearer ${authToken}`,
    ...
    // Get orders
    const ordersResponse = await request.get(
      "/api/orders?status=PENDING&active=true&limit=20",
      {
        headers: {
          "x-company-id": companyId,
          Authorization: `Bearer ${authToken}`,
    ...
    vehicles.forEach((v, i) => {
      ...
      console.log(
        `    Start: [${vroomVehicle.start.join(", ")}] (SAME AS DEPOT)`,
      );
    ...
    orders.slice(0, 5).forEach((o, i) => {
      const vroomJob = {
        id: i + 1,
        location: [
          parseFloat(o.longitude || "0"),
          parseFloat(o.latitude || "0"),
        ],
        service: 300,
      ...
      console.log(
        `    Delivery requirements: [${vroomJob.delivery.join(", ")}]`,
      );
    });
  ...
  test("Debug: Check if VROOM service is available", async ({ request }) => {
    ...
    try {
      ...
      console.log(
        "The fallback algorithm may cause single-vehicle assignment issue!\n",
      );
...
test.describe("UI Flow Debugging", () => {
  test("Navigate through planificacion flow and capture state", async ({
    page,
  }) => {
    console.log("\n=== UI FLOW TEST ===");
    ...
    // Wait for navigation
    await page
      .waitForURL("**/planificacion**", { timeout: 10000 })
      .catch(() => {
    ...
    await page.screenshot({
      path: "tests/e2e/screenshots/1-planificacion-vehicles.png",
    });

    // Wait for vehicles to load
    await page
      .waitForSelector("text=Vehículos", { timeout: 10000 })
      .catch(() => {
    ...
    await page.screenshot({
      path: "tests/e2e/screenshots/2-vehicles-selected.png",
    });
