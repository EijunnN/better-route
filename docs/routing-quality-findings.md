...
> harness hoy corre 29 escenarios solo-VROOM
> (`src/tests/routing-quality/`). Valioso como historia de por qué existe
> el verifier; **no** como descripción del sistema actual.

> Este documento es el **análisis ejecutivo**. Para el estado actual corré el harness (`bun run src/tests/routing-quality/run.ts`): regenera el reporte agregado y deja los JSON por escenario en `results/routing-quality/`.
