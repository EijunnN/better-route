...
export async function POST(
  ...
  try {
      ...
      if (!orderFound) {
        ...
        if (unassignedIndex !== -1) {
          ...
          targetRoute.stops.push({
            ...
            // La no-asignada puede no traer coordenadas; el 0,0 sería una
            // posición real en el Golfo de Guinea, así que se propaga el
            // hueco y lo resuelve la reoptimización que sigue.
            latitude: unassigned.latitude ?? 0,
            longitude: unassigned.longitude ?? 0,
          });
    ...
    // Sin esto, un plan con la forma equivocada entraba en silencio y sólo
    // explotaba días después, al confirmar, con un error que no señalaba a
    // este endpoint.
    try {
      assertPersistableVerifiedPlan(result as unknown as VerifiedPlan);
    } catch (shapeError) {
      console.error(
        "Reassign produjo un plan con forma inválida; no se persiste:",
        shapeError,
      );
      return NextResponse.json(
        {
          error:
            "La reasignación produjo un plan inválido y no se guardó. El plan anterior queda intacto.",
        },
        { status: 500 },
      );
    }
