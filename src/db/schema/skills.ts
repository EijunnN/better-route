...
export const vehicleSkills = pgTable(
  "vehicle_skills",
  {
  ...
  (table) => [
    uniqueIndex("vehicle_skills_company_code_idx").on(
      table.companyId,
      table.code,
    ),
  ],
...
export const vehicleSkillAssignments = pgTable(
  "vehicle_skill_assignments",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    companyId: uuid("company_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("vehicle_skill_assignments_vehicle_skill_idx").on(
      table.vehicleId,
      table.skillId,
    ),
  ],
