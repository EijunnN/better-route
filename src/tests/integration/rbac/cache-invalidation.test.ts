...
describe("custom role permissions flow through getUserPermissionsFromDB", () => {
  ...
  test("revoking a user's custom role drops its permissions immediately", async () => {
  ...
  test("disabling a role's permission removes it from every user that has the role", async () => {
    const company = await createCompany({
    ...
    await testDb
      ...
      .set({ enabled: false })
      .where(
        and(eq(rolePermissions.id, rp.id), eq(rolePermissions.roleId, role.id)),
      );
