...
describe("optimization runner → VROOM: time window preset resolution", () => {
  ...
  test("RANGE preset populates timeWindowStart/End when order has only the id", async () => {
  ...
  test("EXACT preset derives window from exactTime ± toleranceMinutes", async () => {
    const company = await createCompany({
  ...
  test("explicit timeWindowStart/End on the order override any preset", async () => {
    const company = await createCompany({
