...
import { CATEGORY_LABELS, useRoles } from "./roles-context";
                ...
                <div className="divide-y divide-border">
                  {state.roles.map((role) => (
                    <div
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && actions.setSelectedRole(role)
                      }
                    >
                      <div className="flex items-center justify-between">
                        <div>
                              ...
                              >
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    <strong>{role.name}</strong>. Los usuarios
                                    con este rol perderán los permisos
                                    asociados.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction
                                    onClick={() =>
                                      actions.handleDeleteRole(role.id)
                                    }
            ...
            <div className="rounded-lg border border-border bg-card shadow-sm">
              <div className="border-b border-border px-4 py-3">
                <h2 className="font-semibold text-foreground">
                  {state.selectedRole
                    ? `Permisos: ${state.selectedRole.name}`
                    : "Seleccione un rol"}
                </h2>
              ...
              {!state.selectedRole ? (
                ...
                <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
                  {Object.entries(state.rolePermissions.permissions).map(
                    ([category, perms]) => (
                          ...
                          {!state.selectedRole?.isSystem && canUpdateRole && (
                            <div className="flex gap-2">
                              <button
                                type="button"
                                onClick={() =>
                                  actions.handleToggleAllInCategory(
                                    category,
                                    true,
                                  )
                                }
                            ...
                            >
                              ...
                              <Switch
                                ...
                                onCheckedChange={(checked) =>
                                  actions.handleTogglePermission(
                                    perm.id,
                                    checked,
                                  )
                                }
                                disabled={
                                  state.selectedRole?.isSystem ||
                                  state.savingPermission === perm.id ||
                                  !canUpdateRole
                                }
                              />
                            </div>
                  ...
                  {Object.keys(state.rolePermissions.permissions).length ===
                    0 && (
                    <div className="p-8 text-center text-muted-foreground">
